...
from tkinter import scrolledtext, filedialog, messagebox, Text
...
class QADLApp:
    ...
    def create_text_editor(self):
        ...
        # Create line numbers
        self.line_numbers = Text(self.editor_frame, width=4, padx=3, takefocus=0, border=0, background='lightgrey', state='disabled')
        self.line_numbers.pack(side=tk.LEFT, fill=tk.Y)

        # Create main text editor
        self.script_input = scrolledtext.ScrolledText(self.editor_frame, height=30, width=80, undo=True)
        self.script_input.pack(side=tk.RIGHT, expand=True, fill=tk.BOTH)
        self.script_input.bind('<KeyRelease>', self.on_text_change)
    ...
    def update_line_numbers(self):
        ...
        line_count = self.script_input.index('end-1c').split('.')[0]
